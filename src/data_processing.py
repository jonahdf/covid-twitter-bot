...
class DataSets:
    ...
    def get_hosps(self, from_csv):
        ...
        hhs_data = pd.DataFrame.from_records(hosp_results)[['state', 'date', 'inpatient_beds_used_covid']]
...
"""
def get_state_hospitalizations(state_codes, dataset, start_date = pd.Timestamp(2020,1,1), end_date = pd.Timestamp.today(), normalize=True):
...
"""
def get_us_hospitalizations(dataset, start_date = pd.Timestamp(2020,1,1), end_date = pd.Timestamp.today(), normalize = True):
    curr_date = start_date
    ...
    while(curr_date <= end_date and curr_date <= max_date):
        ...
        if normalize:
            hosp_sum = day_data.inpatient_beds_used_covid.sum() / sum(definitions.populations.values())* 1000000
        else:
            hosp_sum = day_data.inpatient_beds_used_covid.sum()
...
"""
def get_state_positivity(state_codes, dataset, start_date = pd.Timestamp(2020,1,1), end_date = pd.Timestamp.today()):
    test_data_state = dataset[dataset.state.isin(state_codes)] # Get only data from input State
...
"""
def get_us_positivity(dataset, start_date = pd.Timestamp(2020,1,1), end_date = pd.Timestamp.today()):
    curr_date = start_date
    max_date = dataset.date.max()
    lst = []
    while (curr_date <= end_date and curr_date <= max_date):
...
def get_all_state_rt(dataset, avg=True):
    ...
    for state in definitions.states.keys():
        data = get_state_hospitalizations(state_codes=[state], start_date=(pd.Timestamp.today() - pd.Timedelta(days=20)).date(), dataset=dataset)
        if data.empty:
            rt = False
        else:
...
"""
def get_state_data_weekly(state_codes, dataset, start_date = pd.Timestamp(2020,1,1), end_date = pd.Timestamp.today(), normalize=True, colName = 'new_cases'):
    curr_date = start_date
    input_states = [definitions.states[s] for s in state_codes]
    state_data = dataset[dataset.state.isin(state_codes)][:]
    ...
    while(curr_date <= end_date and curr_date <= max_date):
        ...
        newRow = {'week_start_date': curr_date - datetime.timedelta(6), 'week_end_date': curr_date, colName: data_sum}
        lst.append(newRow)
        curr_date += datetime.timedelta(1)
    return pd.DataFrame(lst)
...
returns:
 df with 'week_start_date', 'week_end_date', and data
"""
def get_all_state_data_weekly(dataset, start_date = pd.Timestamp(2020,1,1), end_date = pd.Timestamp.today(), normalize=True, colName = 'new_cases'):
    return get_state_data_weekly(definitions.states.keys(), dataset, start_date, end_date, normalize, colName)
